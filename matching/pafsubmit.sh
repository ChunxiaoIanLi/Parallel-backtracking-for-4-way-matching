...
do
	...
	do
		sqsub -r $r -q mpi -n 50 -o $path../output_update2/$order.$case /home/ianli/matching/mpi/a.out 0 $path$order.$case.A.pafs.txt $path$order.$case.B.pafs.txt $path$order.$case.C.pafs.txt $path$order.$case.D.pafs.txt tofile
		((case++))
	done
done
